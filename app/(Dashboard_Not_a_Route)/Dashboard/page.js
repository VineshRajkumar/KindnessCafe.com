...
const Page = () => {
  ...
  const username = session.user.email.split("@")[0]; // Get username from session

  return (
    <>
      <DasboardPage username={username} />
    </>
  );
}
...
//         return accumulator + parseInt(currentItem?.amount, 10) || 0; // Safely add amount
//       }, 0);
