...
const PaymentPage = ({ username }) => {
  ...
  //   // // const savedFollowers = currentUser
  ...
  const SaveSupportersOnClick = () => {
    if (supportersform.name.length >= 4 && supportersform.message.length >= 1) {
      ...
      console.log("Done");
      setsupportersform({ name: "", message: "", amount: "" });
      if (one) {
        pay(500);
      } else if (three) {
        pay(1500);
    ...
    } else if (supportersform.name.length < 4) {
      alert("Full Name is Required");
    } else if (supportersform.message.length < 1) {
      alert("Message is Required");
    }
  };

  //follow
  const SaveFollowersOnClick = async () => {
    const newCount = count + 1;
    setCount(newCount);
    await updateFollowers(newCount + currentUser.followers, session.user.email);
  };
  // const SaveFollowersOnClick = async(count) => {
  //   await updateFollowers(count+1,user.session.email)
  ...
  const handleone = () => {
    ...
    setPaymentform({ ...paymentform, amount: "5" });
  };
  const handlethree = () => {
    setthree(true);
    setOne(false);
  ...
  const handlfive = () => {
    ...
    setPaymentform({ ...paymentform, amount: "25" });
  };
  ...
  const pay = async (amount) => {
    ...
    let a = await initiate(amount, username, paymentform, session.user.email);
    let orderId = a.id;

    var options = {
      key: currentUser.razorpayId, // Enter the Key ID generated from the Dashboard
      amount: amountInCents, // Amount is in currency subunits. Default currency is INR. Hence, 50000 refers to 50000 paise
      currency: "USD",
      name: "KindnessCafe", //your business name
      ...
      <div>
        <div className="flex items-center justify-between bg-white border-black shadow-md max-[726px]:justify-end">
          <div className="ml-5 max-[726px]:hidden">
            <div className="flex title-font font-medium items-center md:justify-start justify-center text-gray-900 cursor-pointer p-3">
              <a
              ...
              >
                ...
                <div className="flex flex-col">
                  ...
                  <span className="ml-5 text-sm font-normal text-gray-600">
                    {count + currentUser.followers} supporters
                  </span>
                </div>
              </a>
              <div>
                <button
                  onClick={SaveFollowersOnClick}
            ...
            <div className="flex   items-start gap-x-7 max-[726px]:flex max-[726px]:flex-col max-[726px]:items-center">
              <div className="flex items-center justify-between  ">
                <div className="min-[726px]:hidden">
                  <div className="flex flex-col  title-font font-medium items-center md:justify-start justify-center text-gray-900 cursor-pointer p-3  ">
                    <img
                      src={session.user.image}
                      alt="userimg"
                      width={100}
